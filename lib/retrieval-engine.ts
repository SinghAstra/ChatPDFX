...
export class RetrievalEngine {
  async retrieveChunks(
    ...
    switch (classification.queryType) {
      case "factual":
        ...
        await this.addVectorResults(results, embeddingString, topK);
        await this.addKeywordResults(results, queryKeywords, topK);
        break;

      case "broad":
        // Use vector + tree (summary) retrieval
        await this.addVectorResults(results, embeddingString, topK);
        await this.addSummaryResults(results, embeddingString, topK);
        break;
    ...
    const normalizedResults = this.normalizeAndCombineScores(
      results,
      classification
    );
  ...
  private async addVectorResults(
  ...
  private async addKeywordResults(
    results: RetrievalResult[],
    ...
    >(
      ...
      SELECT 
        ...
        (
          CASE 
            WHEN array_length(c.keywords, 1) IS NULL OR array_length($1::text[], 1) IS NULL THEN 0
            ELSE (
              SELECT COUNT(*)::float 
              FROM unnest(c.keywords) AS ck
              WHERE ck = ANY($1::text[])
            ) / (
              array_length(c.keywords, 1) + array_length($1::text[], 1) - (
                SELECT COUNT(*)::float 
                FROM unnest(c.keywords) AS ck
                WHERE ck = ANY($1::text[])
              )
  ...
  private async addSummaryResults(
    results: RetrievalResult[],
    embeddingString: string,
    topK: number
  ): Promise<void> {
  ...
  private normalizeAndCombineScores(
    results: RetrievalResult[],
    classification: QueryClassification
  ): RetrievalResult[] {
    ...
    const keywordScores = results
      .map((r) => r.keywordScore)
      .filter((s) => s > 0);
    const summaryScores = results
      .map((r) => r.summaryScore)
      .filter((s) => s > 0);
